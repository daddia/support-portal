...
const oneLineBehaviors = [
  ...
  behavior_core.defineBehavior({
    ...
    guard: ({
      ...
      if (!focusTextBlock || !util_splitTextBlock.isTextBlock(snapshot.context, event.block) || !selectionStartPoint || !selectionEndPoint)
        return !1;
  ...
  behavior_core.defineBehavior({
    ...
    guard: ({
      ...
      if (!focusTextBlock || !util_splitTextBlock.isTextBlock(snapshot.context, event.block) || !selectionStartPoint || !selectionEndPoint)
        return !1;
      const blockBeforeStartPoint = util_splitTextBlock.splitTextBlock({
        context: snapshot.context,
        block: focusTextBlock.node,
        point: selectionStartPoint
      })?.before, blockAfterEndPoint = util_splitTextBlock.splitTextBlock({
        context: snapshot.context,
        block: focusTextBlock.node,
      ...
      if (!blockBeforeStartPoint || !blockAfterEndPoint)
        return !1;
      const targetBlock = util_splitTextBlock.mergeTextBlocks({
        context: snapshot.context,
      ...
      }), mergedBlock = util_splitTextBlock.mergeTextBlocks({
        context: snapshot.context,
        targetBlock,
  ...
  behavior_core.defineBehavior({
    ...
    guard: ({
      context,
      event
    }) => event.blocks.filter((block) => util_splitTextBlock.isTextBlock(context, block)).reduce((targetBlock, incomingBlock) => util_splitTextBlock.mergeTextBlocks({
      context,
      targetBlock,
      incomingBlock
    })),
